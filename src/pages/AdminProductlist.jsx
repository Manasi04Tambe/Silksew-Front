...
function AdminProductlist({ updateTotalProducts }) {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const response = await axios.get("https://silksew-back-1.onrender.com/api/products", {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      const response = await axios.delete(`https://silksew-back-1.onrender.com/api/products/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const submitProductData = async (values) => {
    try {
      ...
      let response
      if (isAdding) {
        response = await axios.post("https://silksew-back-1.onrender.com/api/products", productData, {
          headers: {
        ...
        response = await axios.put(`https://silksew-back-1.onrender.com/api/products/${editingProduct._id}`, productData, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data",
          },
